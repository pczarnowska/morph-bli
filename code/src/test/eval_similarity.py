...
def main():
    ...
    # Read embeddings
    src_words, src_matrix = embeddings.read(
        args.src_embeddings, args.encoding, vocabulary=src_vocab, dtype=dtype)
    trg_words, trg_matrix = embeddings.read(
        args.src_embeddings if args.trg_embeddings is None else args.trg_embeddings,
        encoding=args.encoding, vocabulary=trg_vocab, dtype=dtype)

    # Length normalize embeddings so their dot product
    # effectively computes the cosine similarity
    ...
    for i in range(len(golds)):
        ...
        for gold_score, (src, trg) in zip(golds[i], word_pairs[i]):
            try:
                cos = np.dot(
                    src_matrix[src_word2ind[src]], trg_matrix[trg_word2ind[trg]])
                system.append(cos)
    ...
    if len(results) > 1:
        ...
        if args.sim is not None:
            ...
            print('Coverage:{0:7.2%}  Pearson:{1:7.2%}  Spearman:{2:7.2%} | sim.'.
        ...
        if args.rel is not None:
            ...
            print('Coverage:{0:7.2%}  Pearson:{1:7.2%}  Spearman:{2:7.2%} | rel.'.
        ...
        print('Coverage:{0:7.2%}  Pearson:{1:7.2%}  Spearman:{2:7.2%} | all'.
              format(np.mean(results[1]), np.mean(results[2]), np.mean(results[3])))
