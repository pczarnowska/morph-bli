...
def get_args():
    ...
    parser.add_argument('--src_hs', default=400, type=int, help='source encoder hidden dimension')
    parser.add_argument('--trg_hs', default=400, type=int, help='target decoder hidden dimension')
    ...
    parser.add_argument('--merge_tags', default=False, action='store_true')
    parser.add_argument('--add_pos', default=False, action='store_true')
    parser.add_argument('--tag_in_src', default=False, action='store_true')
    parser.add_argument('--unique_tag', default=False, action='store_true')
    parser.add_argument('--predict_pos', default=False, action='store_true')
    parser.add_argument('--split_tag', default=False, action='store_true')
...
class Trainer(object):
    ...
    def load_data(self, dataset, train, dev, arch, test=None, merge_tags=False, config=None):
        assert self.data is None
        logger = self.logger
        # yapf: disable
        if dataset == Data.lemma_tag_class:
            if isinstance(train, list):
                train = train[0]
            self.data = dataloader.LemmaAndTagClassifier(train, dev, test, merge_tags, config)
        ...
        try:
            logger.info('attr vocab %r', self.data.attr)
        except:
            pass
    ...
    def setup_training(self, optimizer, lr, min_lr, momentum, cooldown, dataset):
    ...
    def setup_evalutator(self, dataset):
        if dataset == Data.lemma_tag_class:
            self.evaluator = util.MultiOptionsEvaluator(self.data.target, self.data.source)
        else:
            self.evaluator = util.BasicEvaluator(self.data.target)
    ...
    def train(self, epoch_idx, batch_size, max_norm):
        ...
        for src, src_mask, trg, attr in tqdm(
...
def main():
    ...
    opt = get_args()
    prefix = ""
    splits = opt.model.split("/")
    key_name = splits[-4]
    new_name = prefix + key_name
    opt.model = '/'.join(splits[:-4] + [new_name] + splits[-3:])
    print("new model", opt.model)
    ...
    trainer.load_data(
        ...
        merge_tags=opt.merge_tags, config=opt)
    ...
    if opt.load and opt.load != '0':
        with torch.no_grad():
            eval_res = trainer.evaluate(DEV, epoch_idx)
            eval_res = trainer.evaluate(TEST, epoch_idx)
    else:
        with torch.no_grad():
